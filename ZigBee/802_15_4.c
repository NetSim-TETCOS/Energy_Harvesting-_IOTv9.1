...
_declspec(dllexport) int fn_NetSim_Zigbee_Metrics(char* szMetrics)
{
	...
	fprintf (fp,"#IOT Metrics for Energy Harvesting\n");
	fprintf (fp,"Node Name\tStatus\tTime\n");
	for (i = 1; i <= nDeviceCount; i++)
	{
	
	fprintf(fp,"%s\t%s\t%lf\t%lf\n",NETWORK->ppstruDeviceList[i-1]->szDeviceName,
	NetSim_Node_state[i-1], NetSim_Off_Time[i-1]);
	}
	fclose(fp);
